...
export class AddPaymentCustomerId1760067732019 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "provider" ADD "paymentCustomerId" character varying`,
    );
    await queryRunner.query(
      `ALTER TABLE "client" ADD "paymentCustomerId" character varying`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "client" DROP COLUMN "paymentCustomerId"`,
    );
    await queryRunner.query(
      `ALTER TABLE "provider" DROP COLUMN "paymentCustomerId"`,
    );
  }
}
